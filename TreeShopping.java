...
public class TreeShopping {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i<q; i++){
            ...
            int y = yi[i];
            for(int j = 0; j<n; j++){
                double area1 = calcArea(x, y, arr[j].x1, arr[j].y1, arr[j].x2, arr[j].y2);
                double area2 = calcArea(arr[j].x1, arr[j].y1, x, y, arr[j].x3, arr[j].y3);
                double area3 = calcArea(arr[j].x2, arr[j].y2, arr[j].x3, arr[j].y3, x, y);
                //System.out.println(area1);
                //System.out.println(area2);
                //aSystem.out.println(area3);
                if(area1 + area2 + area3 == calcArea(arr[j].x1, arr[j].y1, arr[j].x2, arr[j].y2, arr[j].x3, arr[j].y3)){
                    counter++;
                }
            }
            //System.out.println("");
